...
const getOrderDate = (o: any) => {
  ...
  const handleCompleteOrder = async () => {
      ...
      try {
          await orderService.updateStatus(fulfillmentOrder.id, {
              status: 'completed',
              deliveredCode: fulfillmentCode
          });
      } catch (err) {
          console.error('Complete order failed:', err);
          alert('فشل تنفيذ الطلب على السيرفر، حاول مرة أخرى');
          return;
      }

      setOrders(prev => prev.map(o => 
          o.id === fulfillmentOrder.id 
          ? { ...o, status: 'completed', deliveredCode: fulfillmentCode, fulfillmentType: 'manual' } // Set fulfillmentType to manual
          : o
      ));
  ...
  const handleConfirmCancel = async () => {
      ...
      try {
          ...
          alert('فشل إلغاء الطلب على السيرفر، حاول مرة أخرى');
          return;
      }
      ...
      // 5. Refresh users list from server (source of truth)
      try {
...
try {
  ...
  const handleUpdateBalance = async (type: 'add' | 'deduct') => {
    ...
    // NOTE: The server now logs this adjustment into the user's wallet transactions.
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
           <div className="bg-[#1f212e] w-full max-w-sm rounded-2xl p-6 border border-gray-700 shadow-2xl">
               ...
               <div className="flex gap-3">
                   ...
                   <button onClick={handleCompleteOrder} className="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-emerald-600/20">تأكيد وإرسال</button>
               </div>
           </div>
        </div>
      )}
      ...
      {cancellationOrder && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-[#1f212e] w-full max-w-sm rounded-2xl p-6 border border-gray-700 shadow-2xl">
                  ...
                  <div className="flex gap-3">
                      <button onClick={() => setCancellationOrder(null)} className="flex-1 bg-gray-700 text-white py-3 rounded-xl font-bold">تراجع</button>
                      <button onClick={handleConfirmCancel} className="flex-[2] bg-red-600 hover:bg-red-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-red-600/20">تأكيد الإلغاء</button>
                  </div>
              </div>
          </div>
      )}
